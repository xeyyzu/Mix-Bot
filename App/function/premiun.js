...
const premium = JSON.parse(fs.readFileSync('./Storage/premium.json'))
/**
 * Add premium user.
 * @param {String} userId
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("./Storage/premium.json", JSON.stringify(_dir));
};
 ...
 * @param {Object} _dir
 */
const expiredCheck = (lilychan, msg, _dir) => {
	setInterval(() => {
		let position = null;
		Object.keys(_dir).forEach((i) => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("./Storage/premium.json", JSON.stringify(_dir));
			idny ? lilychan.sendMessage(idny, { text: "Your premium has run out, please buy again." }) : "";
			idny = false;
		}
	}, 1000);
};
